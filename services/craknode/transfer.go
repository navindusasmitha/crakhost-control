...
const transferVersion="0.24.0"

func init(){go func(){mux:=http.NewServeMux();mux.HandleFunc("/health",transferAuth(func(w http.ResponseWriter,r *http.Request){jsonOut(w,200,map[string]any{"name":"CrakNode Transfer","version":transferVersion,"status":"online"})}));mux.HandleFunc("/v1/servers/",transferAuth(transferRouter));addr:=env("CRAKNODE_TRANSFER_LISTEN",":8089");log.Printf("CrakNode transfer v%s listening on %s",transferVersion,addr);if e:=http.ListenAndServe(addr,mux);e!=nil{log.Printf("transfer service stopped: %v",e)}}()}

func transferAuth(next http.HandlerFunc)http.HandlerFunc{return func(w http.ResponseWriter,r *http.Request){token:=os.Getenv("CRAKNODE_TOKEN");if token!=""&&r.Header.Get("Authorization")!="Bearer "+token{jsonOut(w,401,map[string]string{"error":"unauthorized"});return};next(w,r)}}
