...
export async function getSessionUser(token?:string){
  ...
  const {rows}=await db.query(`select u.id,u.name,u.email,u.role,u.credits from sessions s join users u on u.id=s.user_id where s.token_hash=$1 and s.expires_at>now() limit 1`,[tokenHash(token)]);
  return rows[0] ?? null;
}
