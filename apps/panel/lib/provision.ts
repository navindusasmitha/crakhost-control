...
async function chooseNode(nodeId:string|null|undefined,memory:number,cpu:number,disk:number){
  const base=`select n.*,coalesce(sum(s.memory_mb) filter(where s.status<>'deleted'),0)::int used_memory,coalesce(sum(s.disk_mb) filter(where s.status<>'deleted'),0)::int used_disk,coalesce(sum(s.cpu_limit) filter(where s.status<>'deleted'),0)::numeric used_cpu from nodes n left join servers s on s.node_id=n.id`;
  if(nodeId){const q=await db.query(`${base} where n.id=$1 and n.enabled=true group by n.id limit 1`,[nodeId]);return q.rows[0]||null}
  const q=await db.query(`${base} where n.enabled=true group by n.id having n.capacity_memory_mb-coalesce(sum(s.memory_mb) filter(where s.status<>'deleted'),0) >= $1 and n.capacity_disk_mb-coalesce(sum(s.disk_mb) filter(where s.status<>'deleted'),0) >= $2 and n.capacity_cpu-coalesce(sum(s.cpu_limit) filter(where s.status<>'deleted'),0) >= $3 order by (n.capacity_memory_mb-coalesce(sum(s.memory_mb),0)) desc,n.created_at asc limit 1`,[memory,disk,cpu]);return q.rows[0]||null
}
...
export async function provisionServer(i:ProvisionInput){
  const tq=await db.query('select * from server_templates where slug=$1 and enabled=true limit 1',[i.templateSlug]);const t=tq.rows[0];if(!t)throw new Error(`Unsupported template: ${i.templateSlug}`);
  const node=await chooseNode(i.nodeId,i.memoryMb,i.cpu,i.diskMb);if(!node)throw new Error('No live node has enough free capacity');

  // Authenticated preflight prevents creating DB records against a dead or misconfigured node.
  try{await nodeFetchFor(node,'/diagnostics')}catch(e:any){throw new Error(`CrakNode preflight failed on ${node.name}: ${e?.message||'node unavailable'}`)}
  ...
  try{
    await nodeFetchFor(node,`/v1/servers/${identifier}/create`,{method:'POST',body:JSON.stringify({image:t.image,memoryMb:i.memoryMb,cpu:i.cpu,hostPort:port,containerPort:t.internal_port,env:t.environment||{}})});
    await db.query("update servers set status='running',updated_at=now() where id=$1",[s.id]);
    await db.query("insert into service_events(server_id,type,detail) values($1,'provision','Real Docker workload provisioned successfully')",[s.id]);
    return {...s,status:'running',primary_port:port,node_name:node.name};
  }catch(e:any){
