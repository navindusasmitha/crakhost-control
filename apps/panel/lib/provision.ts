...
type ProvisionInput={ownerId:string;name:string;templateSlug:string;memoryMb:number;cpu:number;diskMb:number;nodeId?:string|null;location?:string|null;port?:number|null;planId?:string|null;environment?:Record<string,string>};
...
async function chooseNode(nodeId:string|null|undefined,location:string|null|undefined,memory:number,cpu:number,disk:number){
  ...
  if(nodeId){
    if(drained.has(String(nodeId)))return null;
    const q=await db.query(`${base}
      where n.id=$4
        and n.enabled=true
      ...
      group by n.id
      ${capacityHaving}
      limit 1`,[memory,disk,cpu,nodeId]);
  ...
  if(location&&location!=='auto'){
    ...
    locationWhere=` and (lower(coalesce(n.location,''))=lower($4) or lower(n.name)=lower($4))`;
  }
  const q=await db.query(`${base}
    ...
    ) desc,n.created_at asc`,params);
  return q.rows.find((x:any)=>!drained.has(String(x.id)))||null;
}

async function choosePort(nodeId:string,preferred?:number|null){
  if(preferred){
    const c=await db.query('select 1 from allocations where node_id=$1 and port=$2 limit 1',[nodeId,preferred]);
    if(!c.rowCount)return preferred;
  }
  for(let p=25565;p<=25999;p++){
    const c=await db.query('select 1 from allocations where node_id=$1 and port=$2 limit 1',[nodeId,p]);
...
export async function provisionServer(i:ProvisionInput){
  ...
  const node=await chooseNode(i.nodeId,i.location,i.memoryMb,i.cpu,i.diskMb);
  if(!node){
    throw new Error(i.location&&i.location!=='auto'
      ?`No healthy schedulable node in ${i.location} has enough free capacity`
      :'No healthy schedulable node has enough free capacity');
  }

  try{await nodeFetchFor(node,'/diagnostics')}
  catch(e:any){throw new Error(`CrakNode preflight failed on ${node.name}: ${e?.message||'node unavailable'}`)}

  const port=await choosePort(node.id,i.port);
  ...
  const s=rows[0];
  await db.query('insert into allocations(node_id,ip,port,server_id) values($1,$2,$3,$4)',[node.id,'0.0.0.0',port,s.id]);

  try{
    const env={...(t.environment||{}),...(i.environment||{})};
    await nodeFetchFor(node,`/v1/servers/${identifier}/create`,{method:'POST',body:JSON.stringify({image:t.image,memoryMb:i.memoryMb,cpu:i.cpu,hostPort:port,containerPort:t.internal_port,env})});
    await db.query("update servers set status='running',updated_at=now() where id=$1",[s.id]);
    await db.query("insert into service_events(server_id,type,detail) values($1,'provision',$2)",[s.id,`Real Docker workload provisioned on ${node.name}${node.location?` (${node.location})`:''}`]);
    return {...s,status:'running',primary_port:port,node_name:node.name,node_location:node.location};
  }catch(e:any){
    const msg=String(e?.message||e).slice(0,500);
    await db.query('delete from allocations where server_id=$1',[s.id]).catch(()=>{});
    await db.query("update servers set status='error',updated_at=now() where id=$1",[s.id]).catch(()=>{});
    await db.query("insert into service_events(server_id,type,detail) values($1,'provision.failed',$2)",[s.id,msg]).catch(()=>{});
