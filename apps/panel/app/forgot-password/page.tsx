...
export default function ForgotPassword(){
 ...
 return <div className="authWrap"><form className="authBox" onSubmit={submit}><img className="authLogo49" src={LOGO} alt="CrakHost"/><div className="brand authBrand"><span>CRAK<span style={{color:'#a78bfa'}}>HOST</span></span></div><div className="authTitle">Forgot password?</div><div className="authSub">Enter your account email. We will send a secure 6-digit OTP and a reset link if the account exists.</div>{message&&<div className="notice"><Mail size={14} style={{verticalAlign:'middle'}}/> {message}</div>}{error&&<div className="notice error">{error}</div>}<div className="field"><label>Email address</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} autoComplete="email" placeholder="you@example.com" required/></div><button className="btn authBtn" disabled={busy}>{busy?<Loader2 size={15} className="spin"/>:<KeyRound size={15}/>} {busy?'Sending...':'Send reset code'}</button><div className="authFoot">Remembered your password? <Link href="/login">Back to sign in</Link></div></form></div>
}
