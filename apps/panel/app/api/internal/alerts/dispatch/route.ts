...
export async function POST(req:Request){
  ...
  if(settings.emailEnabled&&recipients.length&&toSend.length){
    const rows=toSend.map(a=>`<div style="padding:12px 0;border-bottom:1px solid #293044"><b style="color:${a.severity==='critical'?'#fb7185':'#fbbf24'}">${esc(a.severity.toUpperCase())}</b> · <b>${esc(a.title)}</b><div style="color:#9aa3b7">${esc(a.detail)}</div></div>`).join('');
    const subject=`[CrakHost] ${toSend.length} active production alert${toSend.length===1?'':'s'}`;
    const text=toSend.map(a=>`${a.severity.toUpperCase()}: ${a.title} - ${a.detail}`).join('\n');
    for(const recipient of recipients){try{deliveries.push({recipient,...await sendDirectEmail(recipient,subject,`<h2>Production alerts</h2>${rows}<p><a href="${esc((process.env.APP_URL||'').replace(/\/$/,'')+'/alerts')}">Open Alert Center</a></p>`,text)})}catch(e:any){deliveries.push({recipient,sent:false,error:String(e?.message||e)})}}
    const sentAt=new Date().toISOString();for(const a of toSend)if(allCurrent[a.key])allCurrent[a.key].lastSent=sentAt;
  }
  if(settings.emailEnabled&&settings.sendResolved&&recipients.length&&resolved.length){
    const subject=`[CrakHost] ${resolved.length} alert${resolved.length===1?'':'s'} resolved`;const text=resolved.map((x:any)=>`RESOLVED: ${x.title||'Production alert'}`).join('\n');const html=`<h2>Production conditions recovered</h2>${resolved.map((x:any)=>`<div style="padding:8px 0"><b>${esc(x.title||'Production alert')}</b> is no longer active.</div>`).join('')}`;
    for(const recipient of recipients){try{deliveries.push({recipient,resolved:true,...await sendDirectEmail(recipient,subject,html,text)})}catch(e:any){deliveries.push({recipient,resolved:true,sent:false,error:String(e?.message||e)})}}
  }
  await db.query(`insert into system_settings(key,value,updated_at) values('alert_dispatch_state',$1,now()) on conflict(key) do update set value=excluded.value,updated_at=now()`,[JSON.stringify({active:allCurrent,lastRun:new Date().toISOString()})]);
