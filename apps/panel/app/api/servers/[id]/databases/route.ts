...
async function liveTelemetry(databaseName:string,username:string){
  try{
    const stats=await db.query(`select datname,numbackends,xact_commit,xact_rollback,blks_read,blks_hit,temp_bytes,deadlocks,stats_reset,pg_database_size(datname)::bigint as size_bytes from pg_stat_database where datname=$1 limit 1`,[databaseName]);
    ...
    const sessions=await db.query(`select pid,state,coalesce(client_addr::text,'local') client_addr,coalesce(application_name,'') application_name,backend_start,query_start,wait_event_type,wait_event from pg_stat_activity where datname=$1 and usename=$2 order by coalesce(query_start,backend_start) desc nulls last limit 8`,[databaseName,username]);
    const lastActivity=sessions.rows.length?String(sessions.rows[0].query_start||sessions.rows[0].backend_start||''):null;
    return {reachable:true,sizeBytes:Number(r.size_bytes||0),connections:Number(r.numbackends||0),commits:Number(r.xact_commit||0),rollbacks:Number(r.xact_rollback||0),cacheHitPct:total>0?Number(((hits/total)*100).toFixed(2)):100,tempBytes:Number(r.temp_bytes||0),deadlocks:Number(r.deadlocks||0),statsReset:r.stats_reset,lastActivityAt:lastActivity,sessions:sessions.rows};
...
async function testLogin(d:any){const started=Date.now();const client=new Client({host:internalHost(),port:Number(d.port||5432),database:d.database_name||d.name,user:d.username,password:d.password_cipher,connectionTimeoutMillis:3500,statement_timeout:3500,application_name:'crakhost-control-test'});try{await client.connect();const{rows}=await client.query('select current_database() database,current_user username,version() version,now() checked_at');return {ok:true,latencyMs:Date.now()-started,...rows[0]}}finally{await client.end().catch(()=>{})}}
...
export async function PUT(req:NextRequest,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    if(action==='test'){
      try{const result=await testLogin(d);await audit(user.id,'database.test','server',server.id,{database:d.database_name||d.name,ok:true,latencyMs:result.latencyMs});return NextResponse.json({ok:true,result})}catch(e:any){await audit(user.id,'database.test','server',server.id,{database:d.database_name||d.name,ok:false});return NextResponse.json({error:e?.message||'Database login failed'},{status:502})}
    }
