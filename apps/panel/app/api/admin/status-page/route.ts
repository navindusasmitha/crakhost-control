...
import {DEFAULT_STATUS_CONFIG,getStatusConfig,normalizeStatusConfig} from '@/lib/status-page';
...
export async function POST(req:Request){
  ...
  const body=await req.json().catch(()=>({}));const action=String(body.action||'');const current=await getStatusConfig();let incidents=[...current.incidents];const now=new Date().toISOString();
  if(action==='create'){
    ...
    const severity=['maintenance','minor','major'].includes(String(body.severity))?body.severity:'minor';
    incidents=[{id:randomUUID(),title,message,severity,status:'investigating',createdAt:now,updatedAt:now,resolvedAt:null},...incidents].slice(0,100);
  }else if(action==='update'){
    const id=String(body.id||'');let found=false;incidents=incidents.map(x=>{if(x.id!==id)return x;found=true;const status=['investigating','identified','monitoring','resolved'].includes(String(body.status))?body.status:x.status;return{...x,title:String(body.title||x.title).trim().slice(0,160),message:String(body.message||x.message).trim().slice(0,2000),severity:['maintenance','minor','major'].includes(String(body.severity))?body.severity:x.severity,status,updatedAt:now,resolvedAt:status==='resolved'?(x.resolvedAt||now):null}});if(!found)return NextResponse.json({error:'Incident not found.'},{status:404});
  }else if(action==='resolve'){
    const id=String(body.id||'');incidents=incidents.map(x=>x.id===id?{...x,status:'resolved',updatedAt:now,resolvedAt:now}:x);
