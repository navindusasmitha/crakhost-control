  ...
  try{
    ...
    const locked=await withOrderProvisionLock(order.id,async()=>{
      try{
        ...
        const msg=String(e?.message||e).slice(0,700);const fail=await db.connect();let refunded=false;
        try{
        ...
        }catch{await fail.query('rollback').catch(()=>{})}finally{fail.release()}
    ...
    if(!locked.value.ok)return NextResponse.json({error:`Test payment succeeded, but provisioning failed and the sandbox invoice was marked refunded: ${locked.value.msg}`,orderId:order.id},{status:502});

    const server=locked.value.server;const base=appBase();
