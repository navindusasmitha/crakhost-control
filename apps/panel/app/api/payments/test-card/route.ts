...
// TEST-ONLY payment simulator. Never charge real cards here.
...
export async function POST(req:NextRequest){
  ...
  try{
    ...
    const plan=rows[0];if(!plan)return NextResponse.json({error:'Plan not found'},{status:404});
    const amount=Number(plan.price_monthly);
    const oq=await db.query(`insert into orders(user_id,plan_id,status,amount,currency,template_slug,server_name,payment_method,paid_at,metadata) values($1,$2,'PAID',$3,$4,$5,$6,'test_card',now(),$7::jsonb) returning *`,[user.id,plan.id,amount,plan.currency,plan.template_slug||'minecraft',serverName,JSON.stringify({testCard:`**** **** **** ${card.slice(-4)}`,gateway:'TEST'})]);
    ...
    try{
      ...
      await db.query("update orders set status='FAILED',failure_reason=$2,updated_at=now() where id=$1",[order.id,msg]);
      return NextResponse.json({error:`Payment simulated successfully, but provisioning failed: ${msg}`,orderId:order.id},{status:502});
    }
  }catch(e:any){console.error(e);return NextResponse.json({error:e.message||'Test payment failed'},{status:500})}
}
