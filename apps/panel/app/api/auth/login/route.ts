...
export async function POST(req:Request){
  try{
    ...
    const {rows}=await db.query('select id,name,email,password_hash,role,credits from users where lower(email)=lower($1) limit 1',[email.trim()]);
    const user=rows[0];
    ...
    const res=NextResponse.json({ok:true,user:{id:user.id,name:user.name,email:user.email,role:user.role,credits:user.credits}});
